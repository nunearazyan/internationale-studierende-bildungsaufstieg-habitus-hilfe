...
#X_test, y_train, y_test = train_test_split(xml_tfidf_nicht_passung_features, y_finance_status, test_size=0.2, stratify=y_finance_status, random_state=42) # Finanzielle Situation in der Familie
...
# Pipeline mit RandomUnderSampler + LR
pipeline = Pipeline([
...
param_grid = {
    'randfor__n_estimators': [100, 250, 500],
    'randfor__max_depth': [None, 20, 30, 40],
    'randfor__criterion': ['gini', 'entropy'],
    'randfor__min_samples_split': [2, 3, 5],
    'randfor__min_samples_leaf': [1, 2]
}
...
# N-Gramme beim Spracherwerb internationaler Student*innen
ngram_columns_c = ngram_count_features.columns.tolist()
get_top_num_features_for_class(df=df, columns=ngram_columns_c, class_column='tn_dt_spr', target_class='Spracherwerb im jungen Alter außerhalb DE', top_n=10)
# Frequenzen der Wörter mit dem höchsten IG bei Staatsangehörigkeit 
...
# Frequenzen der Wörter mit dem höchsten IG bei Aufenthaltsstatus 
...
# Frequenzen der Wörter mit dem höchsten IG beim Spracherwerb internationaler Student*innen
get_top_num_features_for_class(df=df, columns=ig_features, class_column='tn_dt_spr', target_class='Spracherwerb im jungen Alter außerhalb DE', top_n=10)# Frequenzen der Attribute des XML-Tags <sek_habitus> bei Aufenthaltsstatus 
...
# Frequenzen der Attribute des XML-Tags <sek_habitus> bei Staatsangehörigkeit 
...
# Optimale Parameter für Staatsangehörigkeit
pipeline = Pipeline([
    ('under', RandomUnderSampler(random_state=42)),
    ('randfor', RandomForestClassifier(
        criterion='gini',
...
# Optimale Parameter für Aufenthaltsstatus
# pipeline = Pipeline([
#     ('under', RandomUnderSampler(random_state=42)),
#     ('randfor', RandomForestClassifier(
#         criterion='gini',
...
# # Optimale Parameter für Spracherwerb im jungen Alter außerhalb DE
# pipeline = Pipeline([
#     ('under', RandomUnderSampler(random_state=42)),
#     ('randfor', RandomForestClassifier(
#         criterion='gini',
